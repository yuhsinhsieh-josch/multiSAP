...
saveRDS(mmat_filtered, file.path(params$directory, params$sample, "signac", paste0(params$sample, "_mmat_filtered.rds" )))

```

**Mitochondrial variant pseduobulk heteroplasmy rank plot**

The plot below shows the rank pseduobulk heteroplasmy, and the variants with more than pseduobulk heteroplasmy 1% were labeled. Homoplasmic variants were removed by filtering out variant with variant-mean ratio less than 0.01.
```{r, message=FALSE, warning=FALSE, fig.width=4, fig.height=4}
mtVAR.meta <- as.data.frame(rowData(mmat_filtered))
mtVAR.meta %>% filter(vmr >= 0.01) %>% 
  arrange(desc(mean)) %>% mutate(rank = 1:n()) %>% 
  mutate(label = ifelse(mean > 0.01, variant, NA)) %>% 
